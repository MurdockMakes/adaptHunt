...
| Tactical AI | Evaluation cadence, combat range gates, tactical hysteresis/commitment, sequence rules, deterministic seed, near-best score window/temperature, repetition fatigue, recent-result memory, and urgent reaction timing |
...
The native decision defaults use a `0.16` near-best raw-score window and `0.08` selection temperature. Repetition keeps five commits, applies `0.22` immediately plus `0.08` for accumulated repeats, and caps the full channel at `0.42`. Recent outcomes keep six resolved actions, apply `0.08` per repeated failure with only half strength for the first failure, add at most `0.015` per success, and cap at `0.24`. Threat reactions wait `0.18` seconds and then observe a `0.8`-second urgent cooldown. Every value rejects non-finite config input and is hard-clamped before use.
...
| Decision policies | `FEnemyActionSelectionPolicy`, `FEnemyActionRepetitionPolicy`, `FEnemyRecentOutcomePolicy`, `FEnemyUrgentDecisionState` | Provide pure near-best weighted choice, bounded commit fatigue, fixed-window terminal-result modifiers, and committed-phase reaction timing |
